...
namespace Fusee.Tutorial.Core
{
    public class FirstSteps : RenderCanvas
    {
        ...
        public override void Init()
        {
            ...
            // Hintergrundfarbe
            RC.ClearColor = new float4(0.6f, 0.2f, 0.1f, 0.6f);

            // Create a scene with a cube
            // The three components: one XForm, one Material and the Mesh
           
            // Würfel
            // Material des Würfels
            ...
            //Farbe des Würfels
            {
                Diffuse = new MatChannelContainer {Color = new float3(128, 0, 0)}, //rot
            ...
            {
                Diffuse = new MatChannelContainer {Color = new float3(0, 128, 0)}, //grün
                Specular = new SpecularChannelContainer {Color = float3.One, Shininess = 4}
            };
            var cubeMesh1 = SimpleMeshes.CreateCuboid(new float3(5, 5, 5));

            //Würfel2
            _cubeTransform2 = new TransformComponent {Scale = new float3(6, 6, 1), Translation = new float3(20, 10, 10)};
            var cubeMaterial2 = new MaterialComponent
            {
                Diffuse = new MatChannelContainer {Color = new float3(0, 3, 2)}, //türkis
                Specular = new SpecularChannelContainer {Color = float3.One, Shininess = 4}
            };
            var cubeMesh2 = SimpleMeshes.CreateCuboid(new float3(2, 1, 1));
